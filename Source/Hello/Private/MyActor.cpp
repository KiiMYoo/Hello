...
void AMyActor::move()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Start Location: (%d, %d)"), FMath::FloorToInt(newstart.X), FMath::FloorToInt(newstart.Y));
	...
	for (int i = 0; i < 10; i++)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Current Location: (%d, %d)"), FMath::FloorToInt(newstart.X), FMath::FloorToInt(newstart.Y));
